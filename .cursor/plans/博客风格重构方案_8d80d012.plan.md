...
- 调整备注：根据反馈，语言控件从首页首屏 label 行迁移到全站 masthead 右上角，脚本改为全页面可用的站点级语言状态；导航、日期线和首页首屏 UI 共用同一个 `site-lang` 选择，文章内容和路由结构仍保持不翻译。
- 调整备注：根据反馈，首页首屏移除最新文章 proof card，不再在 hero 区域展示文章推荐；文章列表和文章详情页保持原有入口与内容展示。
